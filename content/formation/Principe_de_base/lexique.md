...
* Chaque couche vectoriell peut comporter plusieurs entités.
* Par exemple une couche **Plan de ferme** peut avoir plusieurs polygone de champ.

## Noeuds
* Dans une entitée vectorielle, chaque point qui définie l'entité est un noeud. 
* Par exemple pour une ligne droite, j'ai un noeud au début et un noeud à la fin, dans un rectangle j'en aurais 4.
* Chaque noeuds est présenter par une paire de coordonnée (Lat 45.24, Lon -72.14)
...
* C'est l'information non spatiale contenu dans la base de donnée qui est associé à l'entité.
...
* .SHP - Stock l'information spatiale
* .DBF - Stock l'information non spatiale
* .PRJ - Stock la projection (pour d'autres logiciels)
* .SHX - Stock l'index
* .CPG - Stock l'encodage (type de caractère)
...
* Le types d'analyse qu'on peut faire vas changer si l'information est sous forme de point, de ligne, de polygone ou de raster
...
* Dans cette formation, nous n'appronfondiront pas les analyses avec des Raster. Ceux-ci sont moins utiles pour l'usage de base en géomatique.
